...
def generate_world(rules: dict[str, Any], seed: int | None = None) -> WorldMap:
    ...
    for global_y in range(size_rules["global_height"]):
        ...
        for global_x in range(size_rules["global_width"]):
            global_layers = {
                layer: _roll_weighted_value(rng, rules["layers"][layer]["global_weights"])
...
def _generate_local_layer_grid(
    ...
    grid = [
        [
            _roll_weighted_value(rng, layer_rules["local_weights"])
            for _x in range(width)
        ]
        for _y in range(height)
    ]
    generation_rules = layer_rules.get("local_generation", {})
    for _iteration in range(generation_rules.get("smoothing_iterations", 0)):
        grid = _smooth_grid(grid)
    max_delta = generation_rules.get("max_neighbor_delta")
    if max_delta is not None:
        grid = _limit_neighbor_delta(grid, max_delta)
    return grid
...
def _smooth_grid(grid: list[list[int]]) -> list[list[int]]:
    return [
        [
            _most_common_neighbor_value(grid, x, y)
...
def _most_common_neighbor_value(grid: list[list[int]], x: int, y: int) -> int:
    values = _neighbor_values(grid, x, y, include_self=True)
    return max(sorted(set(values)), key=values.count)


def _limit_neighbor_delta(grid: list[list[int]], max_delta: int) -> list[list[int]]:
    limited = [row[:] for row in grid]
    for y, row in enumerate(limited):
        for x, value in enumerate(row):
            neighbors = _neighbor_values(limited, x, y, include_self=False)
            if not neighbors:
                continue
            average = round(sum(neighbors) / len(neighbors))
            if value > average + max_delta:
                limited[y][x] = average + max_delta
            elif value < average - max_delta:
                limited[y][x] = average - max_delta
    return limited
...
def _neighbor_values(
    ...
    values: list[int] = []
    for neighbor_y in range(max(0, y - 1), min(len(grid), y + 2)):
        for neighbor_x in range(max(0, x - 1), min(len(grid[0]), x + 2)):
            if not include_self and neighbor_x == x and neighbor_y == y:
                continue
            values.append(grid[neighbor_y][neighbor_x])
    return values
